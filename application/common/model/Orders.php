...
class Orders extends Model{
    ...
    public function getOrderInfoById($id){
        ...
        //查询结果
        $res = Db::table('store_main_orders')->alias('mo')->field('mo.id as order_id,mo.order_no,mo.mode,mode.post_mode,mo.rec_name,mo.address,mo.mobile,pay.payment,mo.express_no,mo.order_status,mo.create_time,mo.delivery_cost,mo.remark,mo.id_no,mo.total_amount,SUM(sub.amount) as amount,SUM(sub.rate_amount) as rate_amount,sub.pro_id')
    ...
    public function getProductInfoById($id){
        ...
        //查询结果
        $res = Db::table('store_sub_orders')->alias('so')->field('so.pro_id,p.p_name,so.count,so.unit_price,p.rate,so.rate_amount,so.amount,con.con_content1,con.con_content2')
